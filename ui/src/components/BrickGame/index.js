...
export default class BrickGame extends Component {
    ...
    drawBall = () => {
		...
		this.canvas.arc(this.ball.x, this.ball.y, this.ball.r, 0, Math.PI*2, true);
    ...
    drawPaddle = () => {
        this.rect(this.paddle.x, this.canvas.h - this.paddle.h, this.paddle.w, this.paddle.h, "#fff");
	}
    ...
    onMouseMove = evt => {
		if (evt.pageX - this.paddle.w / 2 > this.canvas.minx && evt.pageX < this.canvas.maxx) {
    ...
    collision = b => {
		...
		if (distX > (this.brick.w / 2 + this.ball.r)) { return false; }
		if (distY > (this.brick.h / 2 + this.ball.r)) { return false; }

		if (distX <= (this.brick.w / 2)) { return true; } 
		if (distY <= (this.brick.h / 2)) { return true; }
    ...
    draw = () => {
		...
		if(this.active){
			...
			if (row <= this.rows && col <= this.cols){
				// if collision on top
				if (row > 0 && col < this.cols && this.bricks[row - 1][col].active === 1 && this.collision(this.bricks[row - 1][col])){
					this.ball.dy = -this.ball.dy;
					this.bricks[row - 1][col].active = 0;
				}

				// if collision on left
				else if (row < this.rows && col > 0 && this.bricks[row][col - 1].active === 1 && this.collision(this.bricks[row][col - 1])){
					this.ball.dx = -this.ball.dx;
					this.bricks[row][col - 1].active = 0;
				}

				// if collision on right
				else if (row < this.rows && col + 1 < this.cols && this.bricks[row][col + 1].active === 1 && this.collision(this.bricks[row][col + 1])){
					this.ball.dx = -this.ball.dx;
					this.bricks[row][col + 1].active = 0;
				}

				// if collision on bottom
				else if (row + 1 < this.rows && col < this.cols && this.bricks[row + 1][col].active === 1 && this.collision(this.bricks[row + 1][col])){
					this.ball.dy = -this.ball.dy;
					this.bricks[row + 1][col].active = 0;
				}

				// if collision on top left
				else if (row > 0 && col > 0 && this.bricks[row - 1][col - 1].active === 1 && this.collision(this.bricks[row - 1][col - 1])) {
					if (this.ball.dy < 0){
						this.ball.dy = -this.ball.dy;
					}
					if (this.ball.dx < 0) {
				...
				else if (row > 0 && col + 1 < this.cols && this.bricks[row - 1][col + 1].active === 1 && this.collision(this.bricks[row - 1][col + 1])) {
					if (this.ball.dy < 0){
						this.ball.dy = -this.ball.dy;
					}
					if (this.ball.dx > 0) {
				...
				else if (row + 1 < this.rows && col > 0 && this.bricks[row + 1][col - 1].active === 1 && this.collision(this.bricks[row + 1][col - 1])){
					if (this.ball.dy > 0){
						this.ball.dy = -this.ball.dy;
					}
					if (this.ball.dx < 0) {
				...
				else if (row + 1 < this.rows && col + 1 < this.cols && this.bricks[row + 1][col + 1].active === 1 && this.collision(this.bricks[row + 1][col + 1])){
					if (this.ball.dy > 0){
						this.ball.dy = -this.ball.dy;
					}
					if (this.ball.dx > 0) {
			...
			if (this.ball.x + this.ball.dx + this.ball.r > this.canvas.w || this.ball.x + this.ball.dx - this.ball.r < 0) {
				this.ball.dx = -this.ball.dx;
			}
			...
			if (this.ball.y + this.ball.dy - this.ball.r < 0) {
				this.ball.dy = -this.ball.dy;
			} else if (this.ball.y + this.ball.dy + this.ball.r > this.canvas.h - this.paddle.h) {
		...
		for (var i=0; i < this.rows; i++) {
			for (var j=0; j < this.cols; j++) {
				if (this.bricks[i][j].active === 1) {
					this.bricks[i][j].x = (j * (this.brick.w + this.brick.padding)) + this.brick.padding;
					this.bricks[i][j].y = (i * (this.brick.h + this.brick.padding)) + this.brick.padding;
